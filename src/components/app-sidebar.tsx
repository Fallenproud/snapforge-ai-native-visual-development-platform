...
export function AppSidebar(): JSX.Element {
  ...
  const handleNewProject = async () => {
    try {
      const res = await chatService.createSession("New Forge Project");
    ...
    <Sidebar className="border-r border-white/10 bg-slate-950">
      <SidebarHeader className="p-6">
        <Link to="/" className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-brand-cyan to-brand-purple shadow-glow relative group">
            <Sparkles className="h-6 w-6 text-white group-hover:scale-110 transition-transform" />
            <div className="absolute inset-0 rounded-xl bg-brand-cyan/20 animate-pulse -z-10" />
          </div>
      ...
      <SidebarFooter className="p-4 space-y-2">
        <Button 
          ...
          className="w-full bg-gradient-to-r from-brand-cyan to-brand-purple hover:opacity-90 text-white font-semibold border-none"
        >
          <Plus className="mr-2 h-4 w-4" /> New Project
        </Button>
        <div className="mt-2 px-2 text-[10px] uppercase tracking-widest text-muted-foreground/50 font-bold">
          v1.0.4 Premium
        </div>
      </SidebarFooter>
    </Sidebar>
